...
export default function DeliveryPlanPage() {
  ...
  const getDayTypeBadge = (dayType: string) => {
    ...
    return <Badge variant="primary" size="sm" style={{ backgroundColor: colors[dayType] }}>{dayType}</Badge>;
  };
  ...
  const columns = [
    ...
    { key: 'planNo', label: 'Plan No', render: (item: DeliveryPlan) => <span className="font-mono font-semibold" style={{ color: '#C8102E' }}>{item.planNo}</span> },
    ...
    { key: 'actions', label: 'Actions', render: (item: DeliveryPlan) => (
      <div className="flex items-center space-x-2">
        ...
        {item.status === 'Draft' && <button className="p-1.5 rounded transition-colors" style={{ color: '#6B7280' }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#F9FAFB'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'} title="Edit"><Edit className="w-4 h-4" /></button>}
      </div>
    )},
  ];
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold" style={{ color: '#111827' }}>Delivery Plan Creation</h1>
          <p className="mt-1" style={{ color: '#6B7280' }}>Create and manage delivery plans with auto-loaded defaults ({filteredPlans.length} plans)</p>
        </div>
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Create Delivery Plan" size="md">
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-lg" style={{ backgroundColor: '#FFFBEB', border: '1px solid #FFD100' }}>
            <p className="text-sm font-medium mb-2" style={{ color: '#78350F' }}>Auto-Load Information</p>
      ...
      <Modal isOpen={showViewModal} onClose={() => { setShowViewModal(false); setSelectedPlan(null); }} title="Delivery Plan Details" size="md">
        {selectedPlan && (
          <div className="space-y-4">
            ...
            <div><p className="text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Plan Date</p><p className="text-sm" style={{ color: '#111827' }}>{new Date(selectedPlan.planDate).toLocaleDateString()}</p></div>
            <div className="grid grid-cols-2 gap-4">
              <div><p className="text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Day Type</p>{getDayTypeBadge(selectedPlan.dayType)}</div>
              <div><p className="text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Delivery Turn</p><p className="text-sm font-semibold" style={{ color: '#111827' }}>{selectedPlan.deliveryTurn}</p></div>
            </div>
            <div><p className="text-xs font-medium mb-1" style={{ color: '#6B7280' }}>Created By</p><p className="text-sm" style={{ color: '#111827' }}>{selectedPlan.createdBy} • {selectedPlan.createdAt}</p></div>
            
            <div className="pt-4 border-t" style={{ borderColor: '#E5E7EB' }}>
              <p className="text-xs font-medium mb-2" style={{ color: '#6B7280' }}>Workflow Status</p>
              <div className="flex items-center space-x-2">
                {['Draft', 'Confirmed', 'InProduction', 'Delivered'].map((s, i) => (
                  <div key={s} className="flex items-center">
                    <div className={`px-3 py-1 rounded text-xs font-medium ${selectedPlan.status === s ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'}`}>{s}</div>
                    {i < 3 && <div className="w-6 h-px mx-1" style={{ backgroundColor: '#D1D5DB' }}></div>}
                  </div>
                ))}
              </div>
            </div>
