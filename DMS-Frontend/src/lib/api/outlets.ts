...
export const outletsApi = {
  async getAll(
    ...
    const response = await apiClient.get<OutletsResponse>(`/api/outlets?${params}`);
    return response.data.data;
  },

  async getById(id: string): Promise<Outlet> {
    const response = await apiClient.get<Outlet>(`/api/outlets/${id}`);
    return response.data.data;
  },

  async create(data: CreateOutletDto): Promise<Outlet> {
    const response = await apiClient.post<Outlet>('/api/outlets', data);
    return response.data.data;
  },

  async update(id: string, data: UpdateOutletDto): Promise<Outlet> {
    const response = await apiClient.put<Outlet>(`/api/outlets/${id}`, data);
    return response.data.data;
  },
