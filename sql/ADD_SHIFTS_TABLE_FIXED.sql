-- =====================================================
-- ADD SHIFTS TABLE AND UPDATE DAILY PRODUCTIONS - FIXED
-- Run this script in pgAdmin to enable shift management
-- =====================================================

-- Step 1: Create the shifts table (without foreign keys first)
...
-- Add foreign keys to users table if it exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
        ...
        IF NOT EXISTS (
            ...
            WHERE constraint_name = 'fk_shifts_created_by'
        ) THEN
            ALTER TABLE shifts 
        ...
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'fk_shifts_updated_by'
        ) THEN
            ALTER TABLE shifts 
...
-- Step 3: Check if daily_productions table exists, if not, skip modifications
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'daily_productions') THEN
        
        -- Add shift_id column if it doesn't exist (temporarily nullable)
        IF NOT EXISTS (
        ...
        -- Migrate existing shift data from enum to shift_id if old shift column exists
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
        ...
        ELSE
            -- If no old shift column, set all to Morning by default
            UPDATE daily_productions
            SET shift_id = '11111111-1111-1111-1111-111111111111'::UUID
            WHERE shift_id IS NULL;
        END IF;

        -- Make shift_id NOT NULL after data migration
        ...
        -- Drop old shift column if it exists and is not shift_id
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'daily_productions' AND column_name = 'shift' AND column_name != 'shift_id'
        ) THEN
...
-- Step 4: Add shift management permissions if permissions table exists
DO $$
BEGIN
...
-- Step 5: Grant shift permissions to Manager role if role exists
DO $$
DECLARE
...
-- Check if daily_productions was updated
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'daily_productions') THEN
        RAISE NOTICE 'Daily productions table exists and was updated with shift_id';
    ELSE
        RAISE NOTICE 'Daily productions table does not exist yet - will be created when you run backend';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'permissions') THEN
        RAISE NOTICE 'Shift permissions added successfully';
    END IF;
END $$;
...
SELECT '✨ You can now manage shifts from Administrator → Shifts menu' as next_step;
